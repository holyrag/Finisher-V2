...
Biiofc.msToDate = (ms) => {
        ...
        return days + " Dayss " + hours + " Hrs " + minutes + " Mins"
}
...
Biiofc.msToTime = (ms) => {
        ...
        return [h + ' Hrs ', m + ' Mins ', s + ' Secs'].map(v => v.toString().padStart(2, 0)).join(' ')
}
    
Biiofc.msToHour = (ms) => {
        let h = isNaN(ms) ? '--' : Math.floor(ms / 3600000)
        return [h + ' Hrs '].map(v => v.toString().padStart(2, 0)).join(' ')
}
    
Biiofc.msToMinute = (ms) => {
        let m = isNaN(ms) ? '--' : Math.floor(ms / 60000) % 60
        return [m + ' Mins '].map(v => v.toString().padStart(2, 0)).join(' ')
}
    
Biiofc.msToSecond = (ms) => {
        let s = isNaN(ms) ? '--' : Math.floor(ms / 1000) % 60
        return [s + ' Secs'].map(v => v.toString().padStart(2, 0)).join(' ')
}
